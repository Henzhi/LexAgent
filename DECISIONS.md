...
| D-M4-2 | **阶段 1 落地语义（2026-09-05）**：`scenes.build_plan()` 把分类结果升级为结构化 `AgentPlan`（场景+工具白名单+确认要求），`ask()/stream()` 写入 state.plan；**工具白名单只对 B 类生效**（`agent_node` 按 `plan.tools` 收窄 schema，`SCENES.tools` 首次被消费；交集为空 fail-open 不收窄），**A 类/未命中 plan.tools 置空 = 全量工具表**（A 类行为与现状逐字一致，回归守卫 `TestAskPathRegressionGuard`）；F12 确认单载荷新增 `tools` 字段；`tool_log`/SSE 工具事件预埋 `agent` 维度（主 Agent 恒 `"main"`，阶段 2 预埋项 C2） | ① A 类也按场景白名单收窄（legal_qa 只留 retrieve/memory）；② 白名单交集为空时报错/硬收窄 | ① 路线图原则一明确「A 类工具表不变、必须零编排」——A 类收窄会让 pkulaw/web 等工具在常见咨询里静默消失，属行为回退；② 场景清单（产品数据）与工具注册表（代码）易脱钩，硬失败违反「分类失败不阻断回答」的 REQ-UW 一贯口径 |
| D-M4-3 | **阶段 2·审核子图（2026-09-05）**：`src/agents/review.py` 三层审核替换旧 validate 的单次 LLM PASS 判定——L1 规则守卫（HallucinationGuard，拦截即短路放行、外层替换 answer 语义不变）；L2 LLM 审核（结构化 verdict JSON，兼容旧 "PASS/理由" 文本，解析失败 fail-open）；L3 **定向回源核验**（仅当答案存在「未回源引用」——引用的《法名》+第X条无检索结果支撑——才调 pkulaw_verify(provision) 对照权威原文，法宝故障/额度耗尽/返回形态不明一律 fail-open）。旧重试语义（validation_passed/retry_count）与图结构零改动；`AGENT_REVIEW_ENABLED=false` 逃生通道动态求值 | ① 每次回答都全量回源核验；② 核验不一致一律硬拒答 | ① pkulaw 按积分计费（200 次/日），全量核验会把日容量砍半——正常回答引用都有内部检索支撑、调用为 0，只有疑似幻觉才花积分（上限 3 条/次）；② 核验故障/形态不明硬拒答违反「辅助组件故障不拖垮主链路」一贯口径（D-M3-8 同款）。**B3 基线锁定主攻方向：可靠性 3.61 与 22 条硬失败（总分≤5），审核子图为收该项而生** |
| D-M4-4 | **审核校准与 ReAct 重生成（2026-09-05，基线 25 条配对数据驱动）**：① L2 审核标准收窄为「只拦硬伤」（幻觉引用/明显法律错误/空转拒答），完整性小缺口只写 feedback 不拒审；② 拒审重生成走 ReAct（`react_retry_prep` 注入审核意见 system 消息 → agent 重入，流式路径重跑循环子图），不再退化到固定管线单次生成；`AGENT_REVIEW_ENABLED=false` 时 retry 仍走旧 generate（逐字兼容，路由按开关动态求值，图双注册两条边） | ① 换审核模型（reasoner 级/异源 API/本地 qwen）；② 误杀后人工兜底 | ① 首轮 25 条实测拒审率 60%（15/25）远超真实硬伤率 ~22%，根因是标准把「遗漏关键信息」当拒审理由——换模型不治标准；候选模型各有硬伤（reasoner 慢贵、qwen3b 判断力不足、新 API 开口子大），若校准后仍不达标再做离线拒审 precision 实验；② 重生成退化路径实证毁掉好答案（#192 基线 14→审核 3）：被拒答案经固定管线单次生成劣化且被放行 |
...
## 离开会话时的生成语义（2026-09-04）

> 背景：用户实测「生成中切换对话 / 切到其他页面 / 刷新页面 → 当前对话仍在消耗 Token，但前端输出消失、变为空白」。根因是 D-M3-12「被动断线继续跑完等重连补发」的前提（前端会 `resume`）在**用户主动离开**时不成立：切会话只 `abort()`（不发 `/chat/cancel`）、刷新/关页面零处置，后端一律判定为"断线等重连"把整轮烧完；而答案落库由前端负责，前端一断就谁也收不到。以下为产品语义与根治方案，用户已在方案权衡表上确认。

| # | 决策 | 与既有决策的关系 | 原因 |
| :--- | :--- | :--- | :--- |
| D-0904-1 | **切换/新建会话不再 abort，改为后台续跑并落库**：`handleSelect`/`handleNewChat` 走 store 的 `switchSession()`（把仍有生成在跑的旧会话现场保活到 `drafts[sid]`），SSE 继续写该会话的消息数组，跑完由 `persistSession(sid, [msg])` 精确落回原会话；切回即见完整答案 | 补强 D-0903-8（把"不中断"从路由切换推广到会话切换）；推翻 2026-09-03「handleSelect/handleNewChat abort」的写法 | 原写法只 `abort()` 不发 `/chat/cancel`——后端按 D-M3-12 仍跑完整轮，但产出既不在前端（store 已清空/切走）也不在服务端（落库靠前端）→ 用户看到"回答空白 + 额度照扣"。单向 abort 是**最差组合**：钱花了、结果丢了。要么真停（cancel），要么让它跑完并落库（本决策）。只有「停止」按钮与登出才真正取消 |
| D-0904-2 | **刷新/关闭页面后自动续流**：进行中的流快照 `{sid, requestId, lastSeq, answer, sources, traces}` 持久化到 sessionStorage（标签页级，500ms 节流写），页面重进时 `onMounted` 先重建在途回答、再调 `/chat/stream/resume` 按 seq 游标续流；快照超 9 分钟（事件日志 TTL 600s 留余量）视为过期丢弃，续流失败降级为「已中断，请重新提问」 | 首次真正用上 D-M3-12 的 resume 通道（此前仅网络抖动分支触发） | 刷新是最高频的丢失场景：不续流，后端正在跑的这一轮永远回不来。用 sessionStorage 而非 localStorage：多标签页各自独立会话，不会把 A 标签页的流续到 B（与 2026-09-03 store 改用 sessionStorage 的约定一致） |
| D-0904-3 | **后端孤儿流宽限回收**：被动断线后给流打 deadline（`STREAM_ORPHAN_GRACE_SECONDS`，默认 30s），`resume` 到达即认领并清除（生成继续跑完），超时无人认领则 worker 立即 `gen.close()` 停止；设为 0 退回旧行为 | 补强 D-M3-12（不改"断线继续跑"的语义，只给它加一个"无人接手就别烧了"的兜底） | 前端续流（D-0904-2）只能覆盖"同一标签页还会回来"的情况；用户关掉标签页/直接关浏览器时前端没有任何处置机会，且此时发 cancel 会与刷新续流冲突（无法可靠区分刷新与关闭）。回收只能由后端兜底——这是根治"白烧"的那一层，不依赖任何前端行为 |

## 知识库版本新鲜度治理（2026-09-07）

> 背景：体检发现同一部法律的新旧版本与重复副本在库中共存且同为 `active`，检索可能召回已废止条文（如刑法无后缀版不含修正案十二，问「民企董事同业经营是否入刑」会答错）。产物：`docs/知识库版本新鲜度体检报告-2026-09-07.md` + `evaluation/scripts/check_law_version_conflicts.py`。

| # | 决策 | 与既有决策的关系 | 原因 |
| :--- | :--- | :--- | :--- |
| D-0907-1 | **失效与重复文档靠 `documents.status` 标记处置，不物理删除**：旧版 `superseded`（带 `superseded_by` 指向保留版），重复/残缺副本 `duplicate`；检索层原本就有 `d.status='active'` 谓词，填数据即生效。判定脚本默认 dry-run，`--apply` 前自动导出备份 JSON | 首次真正启用 `documents.status` / `superseded_by` / `effective_date`——这些字段 DDL 里一直存在但从未被写入，属半截功能 | 法律条文一旦误删不可恢复（重爬成本高、版本可能已下架）；标记可逆、可追溯，且 `superseded_by` 为将来「你引用的条文已被 X 取代」提示留了口子。判错的代价从「永久丢失」降为「改回 active」 |
| D-0907-2 | **duplicate 与 superseded 一律用「逐条正文比对」判定，禁用长度/条文号相似度启发式**：比对前必须①按 `metadata->>'paragraph_index'` 还原原文顺序，②剥离【罪名】标题，③中文/阿拉伯条号归一 | 推翻本项目此前「按文件名与文件头判断版本」的做法（初版报告据此得出 24 组全冲突的错误结论） | 实证两次打脸：chunk 的 `id` 是 uuid 无语义顺序，按它拼接会把条文拆碎制造伪差异（曾把 19 组重复误判成版本冲突）；不同爬虫来源给的【罪名】标题（「第347条【走私…罪】」）不剥离会让同一版本判成两版（刑法伪差异 87% → 真差异 8 条）。启发式在「条数相同、长度接近、仅 3 条实质差异」的场景必然误判 |
| D-0907-3 | **入库标题在查重之前归一**（`normalize_document_title`：去首尾空白 + 内部空白压缩） | 新增管道约束，防 D-0907-1 的治理成果被再次覆盖 | 文件名尾部空格（"XX法(2023修订) "）被判为另一部法律重新入库，历史遗留 5 组冗余文档 / 768 chunks，同一条款在召回里出现两份。去重按标题精确匹配，标题不先归一则去重形同虚设 |
