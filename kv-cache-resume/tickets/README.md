...
| 2026-09-12 | **T-07 ✅（AC7 通过）** —— 键计算 + 输入归一化（`kv_cache/prefix.py`，62 例）。**核心裁决：AC7（扰动必须 miss）与 R1（空白归一）表面冲突，靠「归一化改输入、从不已改写键」化解** —— `compute_key` 对 token 一个不归一化，`canonicalize` 是 opt-in 的输入改写且其输出即发送内容（否则键与服务端 KV 脱钩 → REQ-W2 的静默错答案）。设计文档补 §4：7 条归一化规则 + 6 条明确不做 + 4 条拒绝缓存规则，并**记录 3 条被否决的检测规则**（裸 epoch / 裸日期 / 纯数字长串 —— 误报会直接打穿 AC4）。下一步：**T-08**（索引与元数据存储）仍可并行；T-09 依赖 T-06/T-07/T-08，T-08 完成后即可开工 |
| 2026-09-12 | **T-08 ✅** —— 索引与元数据存储（`kv_cache/store.py`，48 例）。守住不变式「`index.json` 要么没有某个 key，要么指向真实存在的文件」；写入 bin→meta→index、删除摘索引→删文件，**两个方向中断都只留孤儿**，由幂等的 `scan_and_repair()` 兜底。原子写 `tmp`+`os.replace`+`fsync`。三处口径修正：① 并发「最后一次写」不可判定 → 改为**`hits` 必须正好 20**（丢更新会暴露）+ 最终态自洽，比原断言更强；② 票面「不删 bin」与「清孤儿 bin」矛盾 → 裁定只有修复路径删 bin；③ 新增 `IndexCorrupted`，坏索引**绝不静默当空**（否则一次解析失败升级成删库）。**E1 三张基础票（T-06/T-07/T-08）全部完成**，T-09 依赖已齐，可开工 |
