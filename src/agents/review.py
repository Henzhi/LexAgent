...
REVIEW_PROMPT = """审核以下法律回答。你是守门员：**只拦硬伤，不因小瑕疵打回**——打回会触发代价较高的重新生成，误杀好答案的代价远大于放过小问题（2026-09-05 校准：旧标准把"遗漏关键信息"也当拒审理由，实测拒审率 60% 且误杀好答案）。
...
仅当出现以下**硬伤**之一才 verdict=reject：
1. 幻觉引用：回答引用的《法名》或条款号在检索依据中无出处，且可判断为虚构（并非合理补充）；
2. 明显法律错误：核心结论与引用的法律依据直接矛盾，或有误导性陈述；
3. 空转拒答：回答为空、答非所问，或仅输出"让我进一步检索"类计划性过渡语。

以下情形一律 verdict=pass（可在 feedback 中一句话提示，不作为打回理由）：
- 遗漏了检索依据中的部分次要信息（完整性小缺口）；
- 表述组织与检索依据不同但法律上成立；
- 补充引用了检索依据之外的其他法律且内容合法有效。
