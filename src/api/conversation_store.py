...
psycopg2 连接非线程安全，锁把并发读写全串行化，是并发瓶颈之一。改为每次
操作从进程级连接池借用（`src.db.pool.db_connection`），写完自动归还：
- 并发读写各自独立连接，互不阻塞；
- 连接失效由池处理，无需 `_ensure_connection` 手写重连；
- 建表 DDL 惰性执行一次（幂等 CREATE IF NOT EXISTS，多线程首跑竞争无害）。
"""
...
class ConversationStore:
    ...
    def _ensure_schema(self, conn) -> None:
        """确保表结构就绪（CREATE/ALTER 均幂等；多线程首跑竞争无害）。"""
