...
# 断线后「孤儿流」宽限期（秒）：客户端断开且有事件日志时，生成会继续跑完以
# 供重连补发；但若超过此时限仍无人重连认领（用户刷新后没回来 / 直接关了页面），
# 判定为孤儿流并立即停止生成，避免 Token 白白烧完一整轮。
# 设为 0 = 关闭回收（保持旧行为：断开后一律跑完）。
STREAM_ORPHAN_GRACE_SECONDS = _safe_int("STREAM_ORPHAN_GRACE_SECONDS", 30)
