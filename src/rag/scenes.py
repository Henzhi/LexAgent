...
分三级是因为中文查询里存在「合同」这类横跨多个场景的通用词——
但 B 类场景（需人工确认，打断问答）**禁止**把裸通用词放进普通关键词：
劳动合同/合同纠纷等词汇遍布普通法律咨询，裸词 1.0 会让海量普通问答
误判进确认流程（历史 Bug 复发两次）。B 类合同起草/审查只由动作强特征词触发；
「第X条」这类正则与强特征词则用来把 A 类场景从通用词中区分出来。
...
SCENES: tuple[Scene, ...] = (
    ...
    Scene(
        ...
        # 有意不含裸「合同/协议」：劳动合同/租赁合同等词遍布普通法律咨询
        # （首页示例「劳动合同到期不续签…」都带「合同」），裸词 1.0 会让大量
        # 普通问答误判进 B 类要确认（历史 Bug 两次复发）。只由动作词触发。
        keywords=(),
        strong_keywords=(
            "起草",
            "起草一份",
    ...
    Scene(
        ...
        # 同 contract_draft：不配裸「合同/协议/条款」普通词，避免普通咨询误判。
        keywords=(),
        strong_keywords=(
            "审查",
            "审核",
